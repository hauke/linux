...
/* Add the LAN port into a bridge with no other port by
...
printk("%s:%i: port: %i, \n", __func__, __LINE__, port);
	...
	if (!dsa_is_cpu_port(ds, port)) {
		macconf = GSWIP_MDIO_PHY_LINK_AUTO |
...
static int gswip_port_vlan_single_remove(struct gswip_priv *priv,
				         struct net_device *bridge, int port,
...
printk("%s:%i: port: %i, state: %i\n", __func__, __LINE__, port, state);
