...
struct intel_ssc_spi_hwcfg {
	unsigned int irnen_r;
	unsigned int irnen_t;
};

struct intel_ssc_spi {
...
static void intel_ssc_spi_writel(const struct intel_ssc_spi *spi, u32 val,
				 u32 reg)
{
	__raw_writel(val, spi->regbase + reg);
}

static void intel_ssc_spi_maskl(const struct intel_ssc_spi *spi, u32 clr,
				u32 set, u32 reg)
{
...
static unsigned int tx_fifo_level(const struct intel_ssc_spi *spi)
{
	u32 fstat = intel_ssc_spi_readl(spi, SPI_FSTAT);
	return (fstat & SPI_FSTAT_TXFFL_M) >> SPI_FSTAT_TXFFL_S;
}

static unsigned int rx_fifo_level(const struct intel_ssc_spi *spi)
{
	u32 fstat = intel_ssc_spi_readl(spi, SPI_FSTAT);
	return fstat & SPI_FSTAT_RXFFL_M;
}

static unsigned int tx_fifo_free(const struct intel_ssc_spi *spi)
{
	return spi->tx_fifo_size - tx_fifo_level(spi);
}

static void rx_fifo_reset(const struct intel_ssc_spi *spi)
{
	u32 val = spi->rx_fifo_size << SPI_RXFCON_RXFITL_S;
	val |= SPI_RXFCON_RXFEN | SPI_RXFCON_RXFLU;
	intel_ssc_spi_writel(spi, val, SPI_RXFCON);
}

static void tx_fifo_reset(const struct intel_ssc_spi *spi)
{
	u32 val = 1 << SPI_TXFCON_TXFITL_S;
	val |= SPI_TXFCON_TXFEN | SPI_TXFCON_TXFLU;
	intel_ssc_spi_writel(spi, val, SPI_TXFCON);
}

static void rx_fifo_flush(const struct intel_ssc_spi *spi)
{
	intel_ssc_spi_maskl(spi, 0, SPI_RXFCON_RXFLU, SPI_RXFCON);
}

static void tx_fifo_flush(const struct intel_ssc_spi *spi)
{
...
static void hw_enter_active_mode(const struct intel_ssc_spi *spi)
{
	intel_ssc_spi_writel(spi, SPI_WHBSTATE_SETEN, SPI_WHBSTATE);
}

static void hw_setup_speed_hz(const struct intel_ssc_spi *spi,
			      unsigned int max_speed_hz)
{
	...
	intel_ssc_spi_writel(spi, brt, SPI_BRT);
}

static void hw_setup_bits_per_word(const struct intel_ssc_spi *spi,
					unsigned int bits_per_word)
{
	u32 bm;
	...
	intel_ssc_spi_maskl(spi, SPI_CON_BM_M, bm, SPI_CON);
}

static void hw_setup_clock_mode(const struct intel_ssc_spi *spi,
				unsigned int mode)
{
	...
	intel_ssc_spi_maskl(spi, con_clr, con_set, SPI_CON);
}
...
static void intel_ssc_spi_hw_init(const struct intel_ssc_spi *spi)
{
	 ...
	 */
	intel_ssc_spi_writel(spi, 1 << SPI_CLC_RMC_S, SPI_CLC);
	...
	intel_ssc_spi_writel(spi, SPI_CON_RUEN | SPI_CON_AEN | SPI_CON_TEN |
	...
	intel_ssc_spi_writel(spi, SPI_WHBSTATE_SETMS | SPI_WHBSTATE_CLR_ERRORS,
				  SPI_WHBSTATE);

	/* Reset GPIO/CS registers */
	intel_ssc_spi_writel(spi, 0, SPI_GPOCON);
	intel_ssc_spi_writel(spi, 0xFF00, SPI_FPGO);
			      ...
			      struct spi_device *spidev, struct spi_transfer *t)
{
	...
	con = intel_ssc_spi_readl(spi, SPI_CON);
	if (t->tx_buf)
		con &= ~SPI_CON_TXOFF;
	else
		con |= SPI_CON_TXOFF;
...
static void tx_fifo_write(struct intel_ssc_spi *spi)
{
	...
	while (spi->tx_todo && tx_free) {
		...
		intel_ssc_spi_writel(spi, data, SPI_TB);
		tx_free--;
	}
}

static void rx_fifo_read_full_duplex(struct intel_ssc_spi *spi)
{
	u8 *rx8;
	u16 *rx16;
	u32 *rx32;
...
static void rx_request(struct intel_ssc_spi *spi)
{
	...
	intel_ssc_spi_writel(spi, rxreq, SPI_RXREQ);
}
...
static irqreturn_t intel_ssc_spi_err_interrupt(int irq, void *data)
{
	struct intel_ssc_spi *spi = data;
	u32 stat = intel_ssc_spi_readl(spi, SPI_STAT);
...
static int transfer_start(struct intel_ssc_spi *spi, struct spi_device *spidev,
...
static const struct intel_ssc_spi_hwcfg spi_xrx = {
	.irnen_r = SPI_IRNEN_R_XRX,
	.irnen_t = SPI_IRNEN_T_XRX,
};

static const struct of_device_id intel_ssc_spi_match[] = {
	{ .compatible = "intel,ase-spi", .data = &spi_xway, },
	{ .compatible = "intel,falcon-spi", .data = &spi_xrx, },
	{ .compatible = "intel,xrx100-spi", .data = &spi_xrx, },
	{},
};
MODULE_DEVICE_TABLE(of, intel_ssc_spi_match);

static int intel_ssc_spi_probe(struct platform_device *pdev)
{
	struct spi_master *master;
	struct resource *res;
	struct intel_ssc_spi *spi;
	const struct intel_ssc_spi_hwcfg *hwcfg;
	...
	match = of_match_device(intel_ssc_spi_match, &pdev->dev);
	if (!match) {
	...
	master = spi_alloc_master(&pdev->dev, sizeof(struct intel_ssc_spi));
	if (!master)
		return -ENOMEM;
	...
	err = devm_request_irq(&pdev->dev, rx_irq, intel_ssc_spi_xmit_interrupt,
			       0, SPI_RX_IRQ_NAME, spi);
	if (err)
		goto err_master_put;

	err = devm_request_irq(&pdev->dev, tx_irq, intel_ssc_spi_xmit_interrupt,
			       0, SPI_TX_IRQ_NAME, spi);
	if (err)
		goto err_master_put;

	err = devm_request_irq(&pdev->dev, err_irq, intel_ssc_spi_err_interrupt,
...
//	master->transfer_one_message = intel_ssc_spi_transfer_one_message;
	...
	id = intel_ssc_spi_readl(spi, SPI_ID);
	...
	dev_info(&pdev->dev,
		"Intel SSC SPI controller (Rev %i, TXFS %u, RXFS %u, DMA %u)\n",
...
static int intel_ssc_spi_remove(struct platform_device *pdev)
{
	struct intel_ssc_spi *spi = platform_get_drvdata(pdev);
...
static struct platform_driver intel_ssc_spi_driver = {
	.probe = intel_ssc_spi_probe,
	.remove = intel_ssc_spi_remove,
	.driver = {
		.name = "spi-intel-ssc",
		.owner = THIS_MODULE,
		.of_match_table = intel_ssc_spi_match,
	},
};
module_platform_driver(intel_ssc_spi_driver);

MODULE_DESCRIPTION("Intel SSC SPI controller driver");
MODULE_AUTHOR("Daniel Schwierzeck <daniel.schwierzeck@gmail.com>");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:spi-intel-ssc");
